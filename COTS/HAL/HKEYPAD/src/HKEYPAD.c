...
#define IS_KEYPAD_BUTTON_PRESSED(BTN)   ((BUTTON_STATE) == BUTTON_PRESSED) /* pressed is pulled down to 0 */

extern const uint8_t HKEYPAD_uint8CharArr[HKEYPAD_NO_OF_ROWS][HKEYPAD_NO_OF_COLS];
extern const uint8_t HKEYPAD_uint8CharCodeArr[HKEYPAD_NO_OF_ROWS][HKEYPAD_NO_OF_COLS];
...
HKEYPAD_enuErrorStatus_t HKEYPAD_enuGetPressedKey(HKEYPAD_structKeypadObject_t* Ptr_structKeypadObj, uint8_t* Ptr_uint8PressedKey)
{
    ...
    uint8_t Local_uint8ButtonState = BUTTON_RELEASED;
    ...
    else 
    {
        for (Local_uint8RowIter = 0; Local_uint8RowIter < HKEYPAD_NO_OF_ROWS; Local_uint8RowIter++)
	    {
	    	...
	    	MDIO_enuSetPinValue(Ptr_structKeypadObj->RowPinsPort, Ptr_structKeypadObj->RowPinsArr[Local_uint8RowIter], MDIO_PIN_LOW);
	    	for (Local_uint8ColIter = 0; Local_uint8ColIter < HKEYPAD_NO_OF_COLS; Local_uint8ColIter++)
	    	{
	    		MDIO_enuGetPinValue(Ptr_structKeypadObj->ColPinsPort, Ptr_structKeypadObj->ColPinsArr[Local_uint8ColIter], &Local_uint8ButtonState);
	    		/* read pin */
	    	...
	    	MDIO_enuSetPinValue(Ptr_structKeypadObj->RowPinsPort, Ptr_structKeypadObj->RowPinsArr[Local_uint8RowIter], MDIO_PIN_HIGH);
